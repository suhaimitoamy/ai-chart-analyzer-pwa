...
Harga: ${p2(p)}`;
  ...
  function downloadPdf() {
    let filteredLogs = logs.filter(l => l.includes("Momentum") || l.includes("Target") || l.includes("Harga") || l.includes("Terdeteksi") || l.includes("Dijebol"));
    if (filteredLogs.length === 0) {
        alert("Belum ada event log untuk diunduh.");
        return;
    }
    let textContent = "--- AMY FX EVENT LOGS ---\n\n" + filteredLogs.join("\n");
    let blob = new Blob([textContent], {type: "text/plain"});
    let url = URL.createObjectURL(blob);
    let a = document.createElement("a");
    a.href = url;
    a.download = `Event_Logs_${Date.now()}.txt`;
    a.click();
